...
pub fn resolve_executable_path(executable: &str) -> Result<PathBuf, String> {
    ...
    if input_path.is_absolute() || trimmed.contains(std::path::MAIN_SEPARATOR) {
        ...
        // On Windows, try adding .cmd/.bat extension if the path doesn't have one
        #[cfg(target_os = "windows")]
        {
            for ext in &[".cmd", ".bat", ".exe"] {
                let with_ext = input_path.with_extension(&ext[1..]);
                if is_file_path(&with_ext) {
                    return Ok(std::fs::canonicalize(&with_ext).unwrap_or(with_ext));
                }
            }
        }
    ...
    // On Windows, prioritize .cmd/.bat/.exe extensions over base filename
    // This prevents finding Unix shell scripts when a Windows-specific version exists
    #[cfg(target_os = "windows")]
    {
        for search_path in runtime_search_paths() {
            for ext in &[".cmd", ".bat", ".exe"] {
                let candidate_with_ext = search_path.join(format!("{}{}", trimmed, ext));
                if is_file_path(&candidate_with_ext) {
                    return Ok(std::fs::canonicalize(&candidate_with_ext).unwrap_or(candidate_with_ext));
                }
            }
        }
    }
