    ...
    while retry_count < max_retries {
        ...
        match AcpConnection::connect(&ws_url).await {
            Ok(mut conn) => {
                ...
                > = HashMap::new();
                let mut pending_market_requests: HashMap<
                    i64,
                    tokio::sync::oneshot::Sender<Result<crate::models::MarketResponse, String>>,
                > = HashMap::new();
                let mut pending_install_requests: HashMap<
                    i64,
                    tokio::sync::oneshot::Sender<Result<crate::models::InstallResponse, String>>,
                > = HashMap::new();
                        ...
                        msg = message_rx.recv() => {
                            match msg {
                                Some(ListenerCommand::UserPrompt { content: prompt, session_id: requested_session_id }) => {
                                    ...
                                    // CLI 模式：target_session_id 为 None 时，创建新 session（不累积历史）
                                    if target_session_id.is_none() {
                                        println!("[listener] CLI mode: creating new session (no history accumulation)");
                                        let session_new_id = next_rpc_id(&mut rpc_id_counter);
                                        session_new_request_id = Some(session_new_id);
                                        session_new_target_id = None;
                                        queued_prompts.push_back((prompt.clone(), None));
                                        ...
                                        if let Err(e) = conn.send_message(session_new_request).await {
                                            println!("[listener] Failed to send session/new for CLI mode: {}", e);
                                            let _ = app_handle.emit(
                                                "agent-error",
                                                json!({
                                                    "agentId": &agent_id,
                                ...
                                Some(ListenerCommand::BrowseMcpMarket { response }) => {
                                    // 发送 /mcp online 命令
                                    if let Some(current_session_id) = &session_id {
                                        let browse_id = next_rpc_id(&mut rpc_id_counter);
                                        let browse_request = build_rpc_request(
                                            browse_id,
                                            "session/prompt",
                                            build_prompt_params(current_session_id, "/mcp online"),
                                        );
                                        if let Err(e) = conn.send_message(browse_request).await {
                                            let _ = response.send(Err(format!(
                                                "Failed to send /mcp online: {}",
                                                e
                                            )));
                                            break;
                                        }
                                        // 将响应通道存储到 pending_market_requests
                                        pending_market_requests.insert(browse_id, response);
                                    } else {
                                        let _ = response.send(Err("Session not ready".to_string()));
                                    }
                                }
                                Some(ListenerCommand::InstallMcp { name, response }) => {
                                    // 发送安装命令
                                    if let Some(current_session_id) = &session_id {
                                        let install_id = next_rpc_id(&mut rpc_id_counter);
                                        let install_request = build_rpc_request(
                                            install_id,
                                            "session/prompt",
                                            build_prompt_params(current_session_id, &format!("/mcp install {}", name)),
                                        );
                                        if let Err(e) = conn.send_message(install_request).await {
                                            let _ = response.send(Err(format!(
                                                "Failed to send /mcp install: {}",
                                                e
                                            )));
                                            break;
                                        }
                                        pending_install_requests.insert(install_id, response);
                                    } else {
                                        let _ = response.send(Err("Session not ready".to_string()));
                                    }
                                }
                                Some(ListenerCommand::ListInstalledMcp { response }) => {
                                    // 发送 /mcp list 命令
                                    if let Some(current_session_id) = &session_id {
                                        let list_id = next_rpc_id(&mut rpc_id_counter);
                                        let list_request = build_rpc_request(
                                            list_id,
                                            "session/prompt",
                                            build_prompt_params(current_session_id, "/mcp list"),
                                        );
                                        if let Err(e) = conn.send_message(list_request).await {
                                            let _ = response.send(Err(format!(
                                                "Failed to send /mcp list: {}",
                                                e
                                            )));
                                            break;
                                        }
                                        pending_market_requests.insert(list_id, response);
                                    } else {
                                        let _ = response.send(Err("Session not ready".to_string()));
                                    }
                                }
                                Some(ListenerCommand::BrowseAgentMarket { response }) => {
                                    // 发送 /agents online 命令
                                    if let Some(current_session_id) = &session_id {
                                        let browse_id = next_rpc_id(&mut rpc_id_counter);
                                        let browse_request = build_rpc_request(
                                            browse_id,
                                            "session/prompt",
                                            build_prompt_params(current_session_id, "/agents online"),
                                        );
                                        if let Err(e) = conn.send_message(browse_request).await {
                                            let _ = response.send(Err(format!(
                                                "Failed to send /agents online: {}",
                                                e
                                            )));
                                            break;
                                        }
                                ...
                                Some(ListenerCommand::InstallAgent { name, response }) => {
                                    // 发送安装命令
                                    if let Some(current_session_id) = &session_id {
                                        let install_id = next_rpc_id(&mut rpc_id_counter);
                                        let install_request = build_rpc_request(
                                            install_id,
                                            "session/prompt",
                                            build_prompt_params(current_session_id, &format!("/agents install {}", name)),
                                        );
                                        if let Err(e) = conn.send_message(install_request).await {
                                            let _ = response.send(Err(format!(
                                                "Failed to send /agents install: {}",
                                                e
                                            )));
                                            break;
                                        }
                                ...
                                Some(ListenerCommand::ListInstalledAgents { response }) => {
                                    // 发送 /agents list 命令
                                    if let Some(current_session_id) = &session_id {
                                        let list_id = next_rpc_id(&mut rpc_id_counter);
                                        let list_request = build_rpc_request(
                                            list_id,
                                            "session/prompt",
                                            build_prompt_params(current_session_id, "/agents list"),
                                        );
                                        if let Err(e) = conn.send_message(list_request).await {
                                            let _ = response.send(Err(format!(
                                                "Failed to send /agents list: {}",
                                                e
                                            )));
                                            break;
                                        }
                            ...
                            match result {
                                Ok(Some(message_text)) => {
                                    ...
                                    for line in message_text.lines() {
                                        ...
                                        // 处理市场浏览请求
                                        if let Some(response) = pending_market_requests.remove(&response_id) {
                                            if let Some(error) = message_json.get("error") {
                                                let _ = response.send(Err(format!(
                                            ...
                                            // 尝试从响应中提取市场数据
                                            // 注意：iFlow CLI 的市场命令是交互式的，不会直接返回 JSON 数据
                                            // 这里我们返回一个空的成功响应，实际的市场展示需要在 iFlow CLI 界面中完成
                                            let _ = response.send(Ok(crate::models::MarketResponse {
                                                success: true,
                                                error: None,
                                                items: Some(json!({
                                                    "message": "市场命令已发送，请在 iFlow CLI 界面中进行交互",
                                                    "interactive": true
                                                })),
                                            }));
                                            continue;
                                        ...
                                        // 处理安装请求
                                        if let Some(response) = pending_install_requests.remove(&response_id) {
                                            if let Some(error) = message_json.get("error") {
                                                let _ = response.send(Err(format!(
                                            ...
                                            let _ = response.send(Ok(crate::models::InstallResponse {
                                                success: true,
                                                error: None,
                                                message: Some("安装命令已发送，请在 iFlow CLI 界面中进行交互".to_string()),
                                            }));
                                            continue;
                                        }
                                    }
