...
export function onSessionListClick(event: MouseEvent) {
  ...
  if (actionBtn) {
    ...
    if (action === 'rename-session') {
      void openRenameSessionModal(sessionId);
      return;
    }
  }
      ...
      <div class="session-item ${session.id === state.currentSessionId ? 'active' : ''}" data-session-id="${session.id}">
        <div class="session-row">
          ...
          <div class="session-actions">
            <button class="btn-session-rename" data-action="rename-session" data-session-id="${session.id}" title="重命名会话">✎</button>
...
export async function renameSession(sessionId: string, newTitle: string) {
  ...
  // 更新会话标题
  session.title = newTitle.trim() || '未命名会话';
...
export function openRenameSessionModal(sessionId: string) {
  import('../../dom').then(({ renameSessionModalEl, renameSessionTitleInputEl }) => {
    if (!state.currentAgentId) {
      showError('请先选择一个Agent');
      return;
    }
    const session = state.sessionsByAgent[state.currentAgentId]?.find((s: any) => s.id === sessionId);
    if (!session) {
      showError('会话不存在');
      return;
    }
    ...
    // 设置当前要重命名的会话ID
    (window as any).currentRenameSessionId = sessionId;

    // 填充当前会话标题
    renameSessionTitleInputEl.value = session.title;

    // 显示弹窗
    renameSessionModalEl.classList.remove('hidden');

    // 聚焦输入框
    setTimeout(() => {
      renameSessionTitleInputEl.focus();
      renameSessionTitleInputEl.select();
    }, 100);
  });
}

export function closeRenameSessionModal() {
  import('../../dom').then(({ renameSessionModalEl }) => {
    renameSessionModalEl.classList.add('hidden');
    delete (window as any).currentRenameSessionId;
  });
}

export async function confirmRenameSession() {
  const { renameSessionTitleInputEl } = await import('../../dom');
  const sessionId = (window as any).currentRenameSessionId;
  ...
  const newTitle = renameSessionTitleInputEl.value.trim();
  if (!newTitle) {
    showError('会话名称不能为空');
    return;
  }
