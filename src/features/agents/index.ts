...
export function updateConnectionStatus(connected: boolean) {
  ...
  // 更新市场按钮状态
  const openMcpMarketBtn = document.getElementById('open-mcp-market-btn') as HTMLButtonElement | null;
  const openAgentMarketBtn = document.getElementById('open-agent-market-btn') as HTMLButtonElement | null;
  
  if (openMcpMarketBtn) {
    openMcpMarketBtn.disabled = !connected;
  }
  if (openAgentMarketBtn) {
    openAgentMarketBtn.disabled = !connected;
  }
}
