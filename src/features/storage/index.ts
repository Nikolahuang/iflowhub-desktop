...
export function createSession(agentId: string, title = '新会话', clearHistory = false): Session {
  const now = new Date();
  return {
    ...
    createdAt: now,
    updatedAt: now,
    acpSessionId: clearHistory ? undefined : generateAcpSessionId(), // 不设置 acpSessionId，强制创建新 session
    source: 'local',
  };
}
